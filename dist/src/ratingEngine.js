...
async function loadThreadRelationships() {
    try {
        const savedRelationships = await browserGet('threadRelationships', '{}');
...
async function saveThreadRelationships() {
    try {
        ...
        await browserSet('threadRelationships', JSON.stringify(threadRelationships));
    } catch (e) {
        console.error('Error saving thread relationships:', e);
    }
}

// Initialize thread relationships on load using an async IIFE
(async () => {
...
function applyFilteringToAll() {
    if (!window.observedTargetNode) return;
    const tweets = window.observedTargetNode.querySelectorAll(TWEET_ARTICLE_SELECTOR);
